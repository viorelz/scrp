...
load_default_config() {
  ...
  CONFIG_backup_dir='/var/lib/db_backups'
  ...
  CONFIG_mysql_dump_host_friendly=$(hostname)
  ...
  CONFIG_mail_address='root@localhost'
  CONFIG_encrypt='no'
  CONFIG_encrypt_password='password0123'

  if [ "$CONFIG_mysql_dump_username" = "" ] && [ "$CONFIG_mysql_dump_password" = "" ]; then
    AUTHDATA=""
  else
    AUTHDATA="--user=${CONFIG_mysql_dump_username} --password=${CONFIG_mysql_dump_password}"
  fi
}
...
parse_configuration () {
	...
	for i in "${CONFIG_table_exclude[@]}"; do
	  ...
	  while read -r; do tmp[z++]="${db}.${REPLY}"; done < <(mysql ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${mysql_opt[@]}" --batch --skip-column-names -e "select table_name from information_schema.tables where table_schema='${db}' and table_name like '${table//\*/%}';")
	done
...
dbstatus() {
  if (( $CONFIG_dryrun )); then
    case "${CONFIG_mysql_dump_compression}" in
	'gzip')
	  echo "dry-running: mysqlshow ${AUTHDATA} --host=${CONFIG_mysql_dump_host} ${opt_dbstatus[@]} | gzip_compression > ${1}${suffix}";
	  ;;
	'bzip2')
	  echo "dry-running: mysqlshow ${AUTHDATA} --host=${CONFIG_mysql_dump_host} ${opt_dbstatus[@]} | bzip2_compression > ${1}${suffix}";
	  ;;
	*)
	  echo "dry-running: mysqlshow ${AUTHDATA} --host=${CONFIG_mysql_dump_host} ${opt_dbstatus[@]} > ${1}${suffix}";
	  ;;
    esac
    return 0;
  else
    case "${CONFIG_mysql_dump_compression}" in
	'gzip')
	  mysqlshow ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt_dbstatus[@]}" | gzip_compression > "${1}${suffix}";
	  return $?
	  ;;
	'bzip2')
	  mysqlshow ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt_dbstatus[@]}" | bzip2_compression > "${1}${suffix}";
	  return $?
	  ;;
	*)
	  mysqlshow ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt_dbstatus[@]}" > "${1}${suffix}";
	  return $?
	  ;;
    esac
  fi
...
fullschema () {
  if (( $CONFIG_dryrun )); then
    case "${CONFIG_mysql_dump_compression}" in
	'gzip')
	  echo "dry-running: mysqldump ${AUTHDATA} --host=${CONFIG_mysql_dump_host} ${opt_fullschema[@]} | gzip_compression > ${1}${suffix}";
	  ;;
	'bzip2')
	  echo "dry-running: mysqldump ${AUTHDATA} --host=${CONFIG_mysql_dump_host} ${opt_fullschema[@]} | bzip2_compression > ${1}${suffix}";
	  ;;
	*)
	  echo "dry-running: mysqldump ${AUTHDATA} --host=${CONFIG_mysql_dump_host} ${opt_fullschema[@]} > ${1}${suffix}";
	  ;;
    esac
    return 0;
  else
    case "${CONFIG_mysql_dump_compression}" in
	'gzip')
	  mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt_fullschema[@]}" | gzip_compression > "${1}${suffix}";
	  return $?
	  ;;
	'bzip2')
	  mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt_fullschema[@]}" | bzip2_compression > "${1}${suffix}";
	  return $?
	  ;;
	*)
	  mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt_fullschema[@]}" > "${1}${suffix}";
	  return $?
	  ;;
    esac
  fi
...
# 	  ## <- get latest differential manifest entry for specified db
  ...
  if [[ "x$CONFIG_mysql_dump_differential" = "xyes" ]] && [[ "x${CONFIG_encrypt}" != "xyes" ]] && (( $activate_differential_backup )) && ((! ($filename_flags & $filename_flag_encrypted) )); then
	  ...
	  if [[ "x$pid" = "x0" ]]; then
		...
		case "${CONFIG_mysql_dump_compression}" in
		'gzip')
		  mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@" | gzip_compression > "$cfname";
		  ;;
		'bzip2')
		  mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@" | bzip2_compression > "$cfname";
		  ;;
		*)
		  mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@" > "$cfname";
		  ;;
		esac
		...
		'gzip')
		  if (( $filename_flags & $filename_flag_gz )); then
			diff <(gzip_compression -dc "${manifest_latest_master_entry[0]}") <(mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@") | gzip_compression > "$cfname";
		  elif (( $filename_flags & $filename_flag_bz2 )); then
			diff <(bzip2_compression -dc "${manifest_latest_master_entry[0]}") <(mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@") | gzip_compression > "$cfname";
		  else
			diff "${manifest_latest_master_entry[0]}" <(mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@") | gzip_compression > "$cfname";
		  fi
		  ;;
		'bzip2')
		  if (( $filename_flags & $filename_flag_gz )); then
			diff <(gzip_compression -dc "${manifest_latest_master_entry[0]}") <(mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@") | bzip2_compression > "$cfname";
		  elif (( $filename_flags & $filename_flag_bz2 )); then
			diff <(bzip2_compression -dc "${manifest_latest_master_entry[0]}") <(mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@") | bzip2_compression > "$cfname";
		  else
			diff "${manifest_latest_master_entry[0]}" <(mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@") | bzip2_compression > "$cfname";
		  fi
		  ;;
		*)
		  if (( $filename_flags & $filename_flag_gz )); then
			diff <(gzip_compression -dc "${manifest_latest_master_entry[0]}") <(mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@") > "$cfname";
		  elif (( $filename_flags & $filename_flag_bz2 )); then
			diff <(bzip2_compression -dc "${manifest_latest_master_entry[0]}") <(mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@") > "$cfname";
		  else
			diff "${manifest_latest_master_entry[0]}" <(mysqldump ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${opt[@]}" "$@") > "$cfname";
		  fi
		  ;;
		esac
	  ...
	  if (( $CONFIG_dryrun )); then
		case "${CONFIG_mysql_dump_compression}" in
		'gzip')
		  echo "dry-running: mysqldump ${AUTHDATA} --host=${CONFIG_mysql_dump_host} ${opt[@]} $@ | gzip_compression > ${cfname}"
		  ;;
		'bzip2')
		  echo "dry-running: mysqldump ${AUTHDATA} --host=${CONFIG_mysql_dump_host} ${opt[@]} $@ | bzip2_compression > ${cfname}"
		  ;;
		*)
		  echo "dry-running: mysqldump ${AUTHDATA} --host=${CONFIG_mysql_dump_host} ${opt[@]} $@ > ${cfname}"
		  ;;
		esac
		return 0;
	  else
...
parse_databases() {
  # bash 4.x version
  #mapfile -t alldbnames < <(mysql ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" --batch --skip-column-names -e "show databases")
  alldbnames=()

  printf "# Parsing databases ... "
  # bash 3.0
  local i;i=0;
  while read -r; do alldbnames[i++]="$REPLY"; done < <(mysql ${AUTHDATA} --host="${CONFIG_mysql_dump_host}" "${mysql_opt[@]}" --batch --skip-column-names -e "show databases")
  unset i
