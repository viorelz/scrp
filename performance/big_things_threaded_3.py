...
def get_folder_size(folder_name):
    ...
    res = p.communicate()
    dir_size = float(res[0].decode("utf-8").split('\t')[0]) / 1024
    return dir_size
...
def main():
    ...
    mail_from = "root@" + h[0].decode("utf-8")
    msg = MIMEText(msg)
    msg['Subject'] = "Big things on " + h[0].decode("utf-8")
